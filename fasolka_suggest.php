...
if($_POST){
    $smak = trim_input($_POST['propozycja']);
    $sql = "INSERT INTO beans_proposals(skrytka, smak) VALUES($id, '$smak')";
    db_statement($sql);
    $_SESSION['sukces'] = "Wysłano propozycję!";
    header("Location: fasolka_suggest.php");
    exit();
}
...
<div class="row row-style">
    ...
    <div class="col-6 col-m-6">
        ...
        if(isset($_SESSION['sukces'])){
            echo "<h1>".$_SESSION['sukces']."</h1>";
            unset($_SESSION['sukces']);
        }
        ?>
        <h2>Zaproponuj nowe smaki fasolek!</h2>
        <p class="narrow">Największe szanse mają propozycje:</p>
        <ul>
            <li>zainspirowane tradycyjnymi polskimi potrawami</li>
            <li>należące do podobnych obszarów zainteresowań co obecne, ale o których zapomnieliśmy</li>
            <li>kreatywne (mogą być abstrakcyjne)</li>
        </ul>
        <form class="form-sleek" action="fasolka_suggest.php" method="POST">


            <input name="propozycja" type="text">
            <input type="submit" value="Wyślij">

        </form>
        <?php
        $sql = "SELECT smak FROM beans WHERE 1 ORDER BY smak";
        $result = db_statement($sql);
        echo "<h3>Lista istniejących fasolek</h3>";
        echo "<ul>";
        while($row = mysqli_fetch_assoc($result)){
            $smak = $row['smak'];
            echo "<li>o smaku ".$smak."</li>";
        }
        echo "</ul>";
        ?>
    </div>
