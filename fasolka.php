...
<div class="col-6 col-m-6">
    ...
    if($_GET['cmd'] == 'edit' AND can_edit_name($_GET['id'])){
        print_name_bean_form($_GET['id'], $id);
    }

    ?>

    <h1>Moja kolekcja fasolek</h1>
    <h2>Masz <span class="bit-bigger colored"><? echo ile_fasolek_na_koncie($id)?></span> z <span class="bit-bigger colored"><? echo ile_wszystkich_fasolek()?></span> dostępnych smaków. (Miejsce w rankingu <span class="colored bit-bigger"><? echo return_ranking($id) ?>.</span>)</h2>
    <?
    for ($i = 1; $i <= 154; $i++) {
        if(($i % 3) == 1) echo "<div class='row'>";
        echo "<div class='col-4'>";
        print_fasolka_state($id, $i);
        echo "</div>";
        if(($i % 3) == 0) echo "</div>";
    }
    if(($i % 3) != 0) echo "</div>";
